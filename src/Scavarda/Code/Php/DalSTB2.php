...
if (!class_exists("clsUIPageBase")) {
    include_once($_folderPhpCore . "clsUIPageBase.php");
}
// Includo la classe di errore Garavot
if (!class_exists("clsError")) {
    include_once($_folderPhpCore . "clsError.php");
}
if (!class_exists("clsManageGXML")) {
    include_once($_folderLibPhp . "clsManageGXML.php");
}
if (!class_exists("clsManageXMLConfig")) {
    include_once($_folderCorePhp . "clsManageXMLConfig.php");
}
...
class clsDALSTBEquipments extends clsUIPageBase
{
    ...
    public function GetListForGrid($start, $limit, $sort, $dir, $gridID)
    {
        ...
        if ($sort == "STBName") {
            $sort = "t_Resources.Name";
        }
        if ($sort == "ModelName") {
            $sort = "t_STBModels.Name";
        }
        if ($sort == "LanguageName") {
            $sort = "t_Languages.Name";
        }
        
        $spName = "sp_STBGetListOfEquipments";
        $spPars = $start . ", " . $limit . ", '" . $sort . " " . $dir . "'";
    ...
    public function ManageEquipment($pars)
    {
        ...
        $spPars = $this->GetIdCurrentLanguage() //p_IDLanguage
            . ", " . $op                      //p_Option
        ...
        // dirotta su stalker
        IF ($STALKER && $op === 5) {
            IF (!class_exists('stalker_api')) {
                INCLUDE_ONCE($_folderLibPhp . $this->_pageStalker);
            }
    ...
    public function GetWindowDetailsHTML($id)
    {
        ...
        $html_generic = "<br><div style='padding-left:10px;font-size:12px;color:#15428B;font-weight:bold'>" . $this->GetTranslation($objID . ".lblDetails", "Informazioni di base apparato")
            . "</div><br>";
        
        $html_generic .= "<table class='detail-of-gvod-asset' cellspacing='0px'>";
        
        $html_generic .= $this->GetHtmlDetailsRow(0, $this->GetTranslation($objID . ".lblDetailsName", "Nome") . ":", $Name);
        $html_generic .= $this->GetHtmlDetailsRow(1, $this->GetTranslation($objID . ".lblDetailsIdentifier", "Identificativo") . ":", $Identifier);
        $html_generic .= $this->GetHtmlDetailsRow(0, $this->GetTranslation($objID . ".lblDetailsSTBModel", "Modello") . ":", $STBModel);
        $html_generic .= $this->GetHtmlDetailsRow(1, $this->GetTranslation($objID . ".lblDetailsSTBLanguage", "Lingua") . ":", $Language);
        $html_generic .= $this->GetHtmlDetailsRow(0, $this->GetTranslation($objID . ".lblDetailsIPAddress", "IP Address") . ":", $IPAddress);
        $html_generic .= $this->GetHtmlDetailsRow(1, $this->GetTranslation($objID . ".lblDetailsHWVersion", "Versione Hardware") . ":", $HWVersion);
        $html_generic .= $this->GetHtmlDetailsRow(0, $this->GetTranslation($objID . ".lblDetailsSWVersion", "Versione Software") . ":", $SWVersion);
        $html_generic .= $this->GetHtmlDetailsRow(1, $this->GetTranslation($objID . ".lblDetailsStatus", "Stato") . ":", $Status);
    ...
    public function GetAndSetSTBDataUsingGService($ipaddress, $port, $rank, $idApplication)
    {
            ...
            if (file_exists($outputpath . $fileNameTMP . ".xml")) {
                ...
                if ($result == "0") {
                    $ret = array(
                        "success" => false,
                        "errors"  => true,
                        "values"  => array("Title" => "Error in GService", "Number" => $result, "Message" => "Error in GService Workflows", "Details" => $message),
                        "check"   => 0,
                    );
                } else {
        ...
        $ret = array(
            ...
            "errors"  => true,
            "values"  => array(
                "Title"   => "Errore in lettura valore di ritorno servizio di cancellazione risorsa",
                "Number"  => -1,
                "Message" => "Timeout nella lettura dati device",
                "Details" => "GSERVICE TIMEOUT",
            ),
        );
    ...
    public function GetHtmlDetailsRow($odd, $field, $value)
    {
        $this->_methodName = __METHOD__;
        if ($odd == 1) {
            $style = "style='background:#E1E1E1' height='20px'";
        } else {
            $style = '';
        }
        
        $html = "<tr $style>" . "<td width='30px' style='padding:5px'>$field</td>" . "<td style='font-weight: bold;' width='200'>$value</td>" . "</tr>";
